...
func addDailyRecordForManyDays_CsvFile(chanDailyRecord chan<- DailyRecord) {
	...
	for myTime := dateStart; myTime != dateEnd.AddDate(0, 0, 1); myTime = myTime.AddDate(0, 0, 1) {
		...
		// 判斷檔案是否存在
		_, err := os.Lstat("\\\\leapsy-nas3\\CheckInRecord\\" + fileName)
		...
		if err != nil {
			log_info.WithFields(logrus.Fields{
				...
				"fileName": "\\\\leapsy-nas3\\CheckInRecord\\" + fileName,
			}).Info("檔案不存在")
			...
			for {
				...
				log_info.WithFields(logrus.Fields{
					...
					"line[3]":  time,       //time
					"line[4]":  employeeID, //employeeID
					"line[5]":  msg,        //msg
					"dateTime": dateTime,
				}).Info("dailyrecord")

				dailyrecord := DailyRecord{date, utf8Name, cardId, time, employeeID, msg, dateTime}
